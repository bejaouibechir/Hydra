...
from hydra_etl.internal.connector.interface import Connector, Batch
...
class JSONConnector(Connector):
    ...
    def _flatten_records(
        ...
        # Chemin rapide (étape 0-bis) : enregistrements déjà plats et homogènes.
        # json_normalize n'y changerait rien mais coûte l'essentiel du temps
        # d'extraction (deepcopy interne). Voir _is_flat_homogeneous.
        if self._is_flat_homogeneous(records):
            return records
    ...
    # Types de colonne pour lesquels json_normalize + where + to_dict rend les
    # valeurs à l'identique (même type, même valeur). Exclus volontairement :
    # None (devient NaN ou None selon la version de pandas), mélanges int/float
    # (int converti en float), dict (aplati), NaN, listes.
    _FAST_PATH_TYPES = (frozenset({str}), frozenset({bool}), frozenset({int}), frozenset({float}))

    @classmethod
    def _is_flat_homogeneous(cls, records: List[Any]) -> bool:
        """
        Vrai si json_normalize laisserait `records` strictement inchangé :
        - chaque enregistrement est un dict avec exactement les mêmes clés,
          dans le même ordre ;
        - chaque colonne ne contient qu'un seul type parmi str, bool, int, float ;
        - aucun float NaN.
        Parité vérifiée par fuzzing contre l'ancien chemin (bench/json_fast_exp.py).
        """
        first = records[0]
        if not isinstance(first, dict) or not first:
            return False
        keys = list(first)
        types: Dict[str, set] = {k: set() for k in keys}
        for r in records:
        ...
        for k, ts in types.items():
            if frozenset(ts) not in cls._FAST_PATH_TYPES:
                return False
            if float in ts and any(r[k] != r[k] for r in records):  # NaN
                return False
        return True