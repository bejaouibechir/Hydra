...
export function flowToHdr(
  ...
  const transformNodes = order.map(id => get(id)!).filter(n => (n.data.nodeType as string).startsWith('transform_'))
...
function _uiParamsToEngine(op: string, cfg: Record<string, unknown>): Record<string, unknown> {
  switch (op) {
    ...
    case 'sort':        return { by: _parseList(cfg.by as string), ascending: cfg.ascending !== 'false' && cfg.ascending !== false }
...
function _sourceToParams(src: HdrSource): Record<string, unknown> {
  ...
  // SQL
  return { host: conn.host, port: conn.port, database: conn.database, user: conn.user, password: conn.password, query: ext.query ?? '' }
}
...
function _stepToParams(op: string, raw: Record<string, unknown> = {}): Record<string, unknown> {
  switch (op) {
    ...
    case 'sort':         return { by: Array.isArray(raw.by) ? (raw.by as string[]).join(',') : (raw.by ?? ''), ascending: raw.ascending !== false }
...
export function flowToJobModel(
  ...
  const transformNodes = byOrder(n => (n.data.nodeType as string ?? '').startsWith('transform_'))
